...
lda_test <- predict(lda_fit, testing_data, type = "response")
table(lda_test$class,testing_data$loan_status)
```

#Sensitivity, Specificity, and Test Error Rate
```{r}
test_error_rate <- (891+24)/(1711+24+891+11566)
specificity <- 1711/(1711+24)
sensitivity <- 11566/(11566+891)
test_error_rate
specificity
sensitivity
```
...
#Risk Analysis
```{r}
defaults <- (payments * .94)*-.07 * 24
#Expected value of defaults
expected_payment <- (amounts*(1+rate/100) - amounts)* (11566+24)
#The expected amount we will make in total if we loan to everyone
actual_payment <- expected_payment - defaults
actual_payment
```
...
```{r}
library(MASS)
qda_fit <- qda(model_formula, data = testing_data)
qda_test <- predict(qda_fit, testing_data, type = "response")
table(qda_test$class, testing_data$loan_status)
```

#Sensitivity, Specificity, and Test Error Rate
```{r}
test_error_rate <- (553+466)/(2049+466+553+11124)
specificity <- 2049/(2049+466)
sensitivity <- 11124/(11124+553)
test_error_rate
specificity
sensitivity
```
#Risk Analysis
```{r}
defaults <- (payments * .94)*-.07 * 466
#Expected value of defaults
expected_payment <- (amounts*(1+rate/100) - amounts)* (466 + 11124)
#The expected amount we will make in total if we loan to everyone
actual_payment <- expected_payment - defaults
actual_payment
```
