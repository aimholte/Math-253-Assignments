...
```{r}
unique(Loans_100k$loan_status)
#Loans_100k$loan_status <- ifelse(Loans_100k$statsu == "My first categore", "bankrupt", "default")
#keepers <- Loans_100k$loan_status %in% c("Late (31", "the other one")
...
```
```{r}
model_formula <- as.numeric(loan_status) ~ loan_amnt + int_rate + grade + annual_inc + total_pymnt
...
# wrong mean(testing_data$loan_status == (probability > .5))
```
```{r}
test_error_rate <- (1792+70)/(2532+70+1792+9798)
specificity <- 2532/(2532+70) #What percentage of the people that the model predicts would default that actually defaulted
sensitivity <- 9798/(1792+9798) #What percentage of the people that would have fully paid that the model predicts would have paid
specificity
sensitivity
test_error_rate
```
...
(payments * .94)*-.07 #The average loss of a loan, assuming that half of interest payments are paid and 95% of the principal
(payments * .94)*-.07 * 70 #Expected value of defaults
amounts*(1+rate/100) - amounts
(amounts*(1+rate/100) - amounts)* (9798+70)
sd_payments
sd_payments/sd_amounts #Variance is greater on payments than amounts
