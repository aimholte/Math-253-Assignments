...
#library(gbm)
#boosting_fit <- gbm(Direction ~ . - Year - Today, data = Weekly_train, n.trees = 5000)
#boost_probs <- predict(boosting_fit, Weekly_test, type = "response")
#boost_pred <- ifelse(boost_probs > 0.5)
...
```
```{r}
rf_probs <- predict(randomforest_fit, Weekly_test, type = "response")
